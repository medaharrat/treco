...
export function GoalsPage() {
  ...
  return (
    <div>
      <h1 className="af-h1">Goals</h1>
      <p className="af-subtitle">Optional, informational targets - not a scoreboard. Nothing here is meant to guilt you.</p>

      <Card>
        {goals.length === 0 ? (
          <EmptyState title="No goals set" description="Set an optional weekly or monthly target below to track progress toward it." />
        ) : (
          <div className="af-flex-col af-gap-4">
            {progresses.map(({ goal, current, ratio }) => (
              <div key={goal.id}>
                ...
                <p className="af-muted af-mt-2" style={{ fontSize: 12.5, margin: 0 }}>
      ...
      <Card>
        <h2 className="af-h2">Add a goal</h2>
